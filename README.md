...
![Entries](https://img.shields.io/badge/entries-4%2C841-5a31d8)
...
**4,841 entries covering 4,654 distinct CVEs, spanning 2010 to 2026** - organized by the six layers of the stack, top to bottom:

| Layer | What it covers | Entries |
| --- | --- | ---: |
| `ai-serving` | Inference servers, training frameworks, model formats | 355 |
| `container-orchestration` | Container runtimes, Kubernetes, schedulers, service mesh | 482 |
| `control-plane` | Cluster management, storage, CI/CD, observability | 820 |
| `kernel-hypervisor` | Host kernel, userspace, virtualization, microcode | 854 |
| `gpu-stack` | GPU drivers, firmware, CUDA, container toolkit, vGPU, ROCm, Gaudi | 1,230 |
| `firmware-bmc-fabric` | BMC/IPMI/Redfish, BIOS/UEFI, NVLink, InfiniBand, DPUs, PDUs, cooling | 1,100 |
...
The field that makes this more than an advisory mirror. A CVSS score tells you how bad a vulnerability is; it does not tell you whether fixing it costs a config change or a firmware flash across every node you own. 3,365 entries carry a `fleet.pain_class`, from cheapest to most disruptive:

| Class | Entries | What it means |
| --- | ---: | --- |
| `hot-patch` | 78 | Fixable without interrupting workloads |
| `daemon-restart` | 445 | Service restart on affected nodes |
...
| `unpatchable / mitigate-only` | 125 | **No vendor fix exists** |
