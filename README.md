...
![Entries](https://img.shields.io/badge/entries-4%2C648-5a31d8)
...
**4,648 entries covering 4,461 distinct CVEs, spanning 2010 to 2026** - organized by the six layers of the stack, top to bottom:

| Layer | What it covers | Entries |
| --- | --- | ---: |
| `ai-serving` | Inference servers, training frameworks, model formats | 306 |
| `container-orchestration` | Container runtimes, Kubernetes, schedulers, service mesh | 459 |
| `control-plane` | Cluster management, storage, CI/CD, observability | 785 |
| `kernel-hypervisor` | Host kernel, userspace, virtualization, microcode | 804 |
| `gpu-stack` | GPU drivers, firmware, CUDA, container toolkit, vGPU, ROCm, Gaudi | 1,216 |
| `firmware-bmc-fabric` | BMC/IPMI/Redfish, BIOS/UEFI, NVLink, InfiniBand, DPUs, PDUs, cooling | 1,078 |
...
The field that makes this more than an advisory mirror. A CVSS score tells you how bad a vulnerability is; it does not tell you whether fixing it costs a config change or a firmware flash across every node you own. 3,193 entries carry a `fleet.pain_class`, from cheapest to most disruptive:

| Class | Entries | What it means |
| --- | ---: | --- |
| `hot-patch` | 74 | Fixable without interrupting workloads |
| `daemon-restart` | 343 | Service restart on affected nodes |
| `node-drain` | 195 | Tenant workloads evicted from each node |
| `node-reboot` | 1,842 | Full reboot of each affected node |
...
| `unpatchable / mitigate-only` | 119 | **No vendor fix exists** |
