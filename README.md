...
![Entries](https://img.shields.io/badge/entries-5%2C375-5a31d8)
...
**5,375 entries covering 5,293 distinct CVEs, spanning 2010 to 2026** - organized by the six layers of the stack, top to bottom:

| Layer | What it covers | Entries |
| --- | --- | ---: |
| `ai-serving` | Inference servers, training frameworks, model formats | 405 |
| `container-orchestration` | Container runtimes, Kubernetes, schedulers, service mesh | 551 |
| `control-plane` | Cluster management, storage, CI/CD, observability | 964 |
| `kernel-hypervisor` | Host kernel, userspace, virtualization, microcode | 1,085 |
| `gpu-stack` | GPU drivers, firmware, CUDA, container toolkit, vGPU, ROCm, Gaudi | 1,210 |
| `firmware-bmc-fabric` | BMC/IPMI/Redfish, BIOS/UEFI, NVLink, InfiniBand, DPUs, PDUs, cooling | 1,160 |
...
The field that makes this more than an advisory mirror. A CVSS score tells you how bad a vulnerability is; it does not tell you whether fixing it costs a config change or a firmware flash across every node you own. 3,892 entries carry a `fleet.pain_class`, from cheapest to most disruptive:

| Class | Entries | What it means |
| --- | ---: | --- |
| `hot-patch` | 82 | Fixable without interrupting workloads |
| `daemon-restart` | 656 | Service restart on affected nodes |
| `node-drain` | 197 | Tenant workloads evicted from each node |
| `node-reboot` | 2,181 | Full reboot of each affected node |
