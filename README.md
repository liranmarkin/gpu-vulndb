...
![Entries](https://img.shields.io/badge/entries-5%2C258-5a31d8)
...
**5,258 entries covering 5,170 distinct CVEs, spanning 2010 to 2026** - organized by the six layers of the stack, top to bottom:

| Layer | What it covers | Entries |
| --- | --- | ---: |
| `ai-serving` | Inference servers, training frameworks, model formats | 398 |
| `container-orchestration` | Container runtimes, Kubernetes, schedulers, service mesh | 541 |
| `control-plane` | Cluster management, storage, CI/CD, observability | 938 |
| `kernel-hypervisor` | Host kernel, userspace, virtualization, microcode | 1,044 |
| `gpu-stack` | GPU drivers, firmware, CUDA, container toolkit, vGPU, ROCm, Gaudi | 1,200 |
| `firmware-bmc-fabric` | BMC/IPMI/Redfish, BIOS/UEFI, NVLink, InfiniBand, DPUs, PDUs, cooling | 1,137 |
...
The field that makes this more than an advisory mirror. A CVSS score tells you how bad a vulnerability is; it does not tell you whether fixing it costs a config change or a firmware flash across every node you own. 3,795 entries carry a `fleet.pain_class`, from cheapest to most disruptive:

| Class | Entries | What it means |
| --- | ---: | --- |
| `hot-patch` | 80 | Fixable without interrupting workloads |
| `daemon-restart` | 619 | Service restart on affected nodes |
| `node-drain` | 191 | Tenant workloads evicted from each node |
| `node-reboot` | 2,133 | Full reboot of each affected node |
| `microcode + reboot` | 54 | Microcode update and a reboot |
| `firmware-flash` | 582 | Firmware flash, usually with the node out of service |
| `physical access` | 2 | Someone has to be at the machine |
| `unpatchable / mitigate-only` | 130 | **No vendor fix exists** |
