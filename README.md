...
![Entries](https://img.shields.io/badge/entries-4%2C807-5a31d8)
...
**4,807 entries covering 4,620 distinct CVEs, spanning 2010 to 2026** - organized by the six layers of the stack, top to bottom:

| Layer | What it covers | Entries |
| --- | --- | ---: |
| `ai-serving` | Inference servers, training frameworks, model formats | 347 |
...
| `kernel-hypervisor` | Host kernel, userspace, virtualization, microcode | 843 |
| `gpu-stack` | GPU drivers, firmware, CUDA, container toolkit, vGPU, ROCm, Gaudi | 1,229 |
...
The field that makes this more than an advisory mirror. A CVSS score tells you how bad a vulnerability is; it does not tell you whether fixing it costs a config change or a firmware flash across every node you own. 3,332 entries carry a `fleet.pain_class`, from cheapest to most disruptive:

| Class | Entries | What it means |
| --- | ---: | --- |
| `hot-patch` | 77 | Fixable without interrupting workloads |
| `daemon-restart` | 426 | Service restart on affected nodes |
| `node-drain` | 196 | Tenant workloads evicted from each node |
| `node-reboot` | 1,883 | Full reboot of each affected node |
