...
![Entries](https://img.shields.io/badge/entries-4%2C696-5a31d8)
...
**4,696 entries covering 4,509 distinct CVEs, spanning 2010 to 2026** - organized by the six layers of the stack, top to bottom:

| Layer | What it covers | Entries |
| --- | --- | ---: |
| `ai-serving` | Inference servers, training frameworks, model formats | 329 |
| `container-orchestration` | Container runtimes, Kubernetes, schedulers, service mesh | 462 |
| `control-plane` | Cluster management, storage, CI/CD, observability | 786 |
| `kernel-hypervisor` | Host kernel, userspace, virtualization, microcode | 814 |
| `gpu-stack` | GPU drivers, firmware, CUDA, container toolkit, vGPU, ROCm, Gaudi | 1,217 |
| `firmware-bmc-fabric` | BMC/IPMI/Redfish, BIOS/UEFI, NVLink, InfiniBand, DPUs, PDUs, cooling | 1,088 |
...
The field that makes this more than an advisory mirror. A CVSS score tells you how bad a vulnerability is; it does not tell you whether fixing it costs a config change or a firmware flash across every node you own. 3,230 entries carry a `fleet.pain_class`, from cheapest to most disruptive:

| Class | Entries | What it means |
| --- | ---: | --- |
| `hot-patch` | 76 | Fixable without interrupting workloads |
| `daemon-restart` | 370 | Service restart on affected nodes |
| `node-drain` | 196 | Tenant workloads evicted from each node |
| `node-reboot` | 1,843 | Full reboot of each affected node |
| `microcode + reboot` | 56 | Microcode update and a reboot |
| `firmware-flash` | 563 | Firmware flash, usually with the node out of service |
| `physical access` | 2 | Someone has to be at the machine |
| `unpatchable / mitigate-only` | 120 | **No vendor fix exists** |
