...
![Entries](https://img.shields.io/badge/entries-5%2C296-5a31d8)
...
**5,296 entries covering 5,210 distinct CVEs, spanning 2010 to 2026** - organized by the six layers of the stack, top to bottom:

| Layer | What it covers | Entries |
| --- | --- | ---: |
| `ai-serving` | Inference servers, training frameworks, model formats | 401 |
| `container-orchestration` | Container runtimes, Kubernetes, schedulers, service mesh | 548 |
| `control-plane` | Cluster management, storage, CI/CD, observability | 950 |
| `kernel-hypervisor` | Host kernel, userspace, virtualization, microcode | 1,045 |
| `gpu-stack` | GPU drivers, firmware, CUDA, container toolkit, vGPU, ROCm, Gaudi | 1,200 |
| `firmware-bmc-fabric` | BMC/IPMI/Redfish, BIOS/UEFI, NVLink, InfiniBand, DPUs, PDUs, cooling | 1,152 |
...
The field that makes this more than an advisory mirror. A CVSS score tells you how bad a vulnerability is; it does not tell you whether fixing it costs a config change or a firmware flash across every node you own. 3,822 entries carry a `fleet.pain_class`, from cheapest to most disruptive:

| Class | Entries | What it means |
| --- | ---: | --- |
| `hot-patch` | 82 | Fixable without interrupting workloads |
| `daemon-restart` | 636 | Service restart on affected nodes |
| `node-drain` | 193 | Tenant workloads evicted from each node |
| `node-reboot` | 2,135 | Full reboot of each affected node |
